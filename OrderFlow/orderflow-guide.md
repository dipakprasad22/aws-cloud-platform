...
# README (for `aws-cloud-platform/orderflow/`)
...
Title: **"Why your checkout shouldn't call payment, inventory, and email in a row — building a resilient event-driven order pipeline on AWS"**
1. Hook — the fragile synchronous checkout and how one slow dependency takes down the whole order.
2. The decoupling insight — publish the event once; let each step react independently.
3. The architecture (diagram) — SNS fan-out to SQS queues, DLQs, EventBridge routing, Step Functions.
4. Fan-out vs orchestration — when to use which (the senior distinction).
5. Resilience demo — kill a consumer, watch the queue hold the message; force a failure, watch compensation.
6. Debuggability — the X-Ray service map drawing the system from real traffic.
7. Cost ($0 serverless) + what's next.

---

# TEARDOWN (optional — it's ~$0, but to stay tidy)
Delete: Step Functions state machine + its Lambdas → EventBridge rules → SQS triggers + consumer Lambdas → 6 queues → SNS subscriptions + topic → intake Lambda → API → DynamoDB table → disable X-Ray tracing.

---

## Where this leads
OrderFlow completes the three core patterns: serverless request/response (ShopFront), always-on HA (MarketBase), and event-driven async (OrderFlow). Project 4 (MediaForge) applies event-driven processing to a media pipeline — S3 events triggering transcoding — and Projects 5-6 move into containers and the secure multi-account capstone.
