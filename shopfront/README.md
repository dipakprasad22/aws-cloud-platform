...
## Cost & teardown
~$0 at portfolio traffic (domain is the only real cost). Teardown order: CloudFront (disable → delete) → S3 bucket → API Gateway → Lambda → DynamoDB → SNS → Route 53 record → ACM cert.

## What I'd improve next
- Codify the whole stack in Terraform (the next version).
- Tighten CORS to the exact CloudFront origin.
- Add WAF on CloudFront and request validation at API Gateway.